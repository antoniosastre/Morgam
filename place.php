...
<form action="place.php" method="GET" class="form-horizontal">
...
if(!empty($_GET['f']) && !empty($_GET['t'])){
	...
	echo "<hr>";
	echo tableOfInterval($_GET['f'], $_GET['t'], 0, 0, $_GET['q']);
}else

if(!empty($_GET['d'])){
	echo "Se muestran los últimos ".$_GET['d']." días de ".$_GET['q'].":";;
	echo "<hr>";
	echo tableOfLast($_GET['d'], 0, 0, $_GET['q']);
}else{
	echo "Se muestran todos los vídeos de ".$_GET['y']." de ".$_GET['q'].":";
	echo "<hr>";
	echo tableOfYear($_GET['y'], 0, 0, $_GET['q']);
}
